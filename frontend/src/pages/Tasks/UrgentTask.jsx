...
const UrgentTask = () => {
  ...
  const { data: tasks = [] } = useGetUrgentTasksQuery();
  console.log(tasks);
  return (
    <div className="w-[26rem] border-none rounded-lg m-3 bg-white shadow-lg ml-6 mt-2">
      {tasks?.length > 0 &&
        tasks?.map((task) => <TaskDetail1 task={task} selectedId={id} />)}

      <hr className="text-gray-300" />

      <div className="w-[23rem] border-2 border-gray-300 rounded-lg shadow-lg flex items-center justify-start mt-10 ml-6 mb-6">
        <RiAddLargeLine
